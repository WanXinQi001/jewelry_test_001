        ...
        <td>
            <table width="100%" border="0" align="center" cellpadding="4" cellspacing="1" bgcolor="#aec3de">
                <tr align="left" bgcolor="#F2FDFF">
                    <td height="30" colspan="17" background="images/table_header.gif">采购信息列表</td>
                </tr>
            </table>
        </td>
    </tr>
                    ...
                    <td style="padding-left:0px; padding-right:0px;">
                        <table width="100%" border="1" align="center" cellpadding="4" cellspacing="1" bgcolor="#ebf0f7">
                            <tr>
                                ...
                                <td width='80' align='center' bgcolor='#ebf0f7'>是否入库</td>
                            ...
                            <c:forEach items="${list }" var="u">
                                ...
                                <tr align="center" bgcolor="#FFFFFF">
                                    ...
                                    <td>
                                        <c:if test="${u.gonghao=='null'||u.gonghao==null}">
                                            wanxinqi
                                        </c:if>
                                        <c:if test="${u.gonghao!='null'}">
                                            ${u.gonghao}
                                        </c:if>
                                    </td>
                                    <td>
                                        <c:if test="${u.xingming=='null'||u.xingming==null}">
                                            wanxinqi
                                        </c:if>
                                        <c:if test="${u.xingming!='null'}">
                                            ${u.xingming}
                                        </c:if>
                                    </td>
                                    <td>
                                        <c:if test="${u.bumen=='null'||u.bumen==null}">
                                            管理员
                                        </c:if>
                                        <c:if test="${u.bumen!='null'}">
                                            ${u.bumen}
                                        </c:if>
                                    </td>
                                    <td align='center' bgcolor='#ebf0f7'>${u.iszf} <c:if test="${u.iszf=='否'}"><input
                                            type="button" value="支付" onClick="zhifu(${u.id},'caigouxinxi')"/></c:if> </td>
                                    <td align='center'>${u.issh}
                                        <c:if test="${u.iszf=='是'&&u.issh=='否'}">
                                            <input type="button" value="入库"
                                                   onclick="javascript:location.href='../sh.jsp?id=${u.id}&yuan=${u.issh}&tablename=caigouxinxi'"/>
                                        </c:if>
                                    </td>
                                    ...
                                    <td align="center">
                                        <c:if test="${u.iszf=='否'}">
                                            ...
                                            <a
                                                    class="btn btn-info btn-small" href="deleteCaigouxinxi.do?id=${u.id }"
                                                    onclick="{if(confirm('确定要删除吗?')){return true;}return false;}">删除</a>
                                        </c:if>
                                  <a
                                            class="btn btn-info btn-small" href="caigouxinxiDetail.do?id=${u.id}">详细</a>
                                    </td>
                                </tr>
                            </c:forEach>
                        </table>
