...
<%
    ...
    // 按日统计，添加筛选条件只查询当月数据
    String dailySql = "SELECT caizhi,  COUNT(caizhi) as bbb, DATE_FORMAT(goumairiqi, '%Y-%m-%d') as date " +
            "FROM shangpingoumai " +
            "WHERE YEAR(goumairiqi) =? AND MONTH(goumairiqi) =? " +
            "GROUP BY caizhi, DATE_FORMAT(goumairiqi, '%Y-%m-%d')";
    ...
    // 按月统计
    String monthlySql = "SELECT caizhi, COUNT(caizhi) as bbb, DATE_FORMAT(goumairiqi, '%Y-%m') as month " +
            "FROM shangpingoumai " +
            "GROUP BY caizhi, DATE_FORMAT(goumairiqi, '%Y-%m')";
    ...
    // 按年统计
    String yearlySql = "SELECT caizhi,  COUNT(caizhi) as bbb, DATE_FORMAT(goumairiqi, '%Y') as year " +
            "FROM shangpingoumai " +
            "GROUP BY caizhi, DATE_FORMAT(goumairiqi, '%Y')";
