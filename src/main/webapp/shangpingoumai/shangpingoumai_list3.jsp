                        ...
                        <table width="100%" border="1" align="center" cellpadding="4" cellspacing="1" bgcolor="#ebf0f7">
                            ...
                            <c:forEach items="${list }" var="u">
                                ...
                                <tr align="center" bgcolor="#FFFFFF">
                                    ...
                                    <td align='center' bgcolor='#ebf0f7'>${u.iszf} <c:if test="${u.iszf=='否'}"><input
                                            type="button" value="支付" onClick="zhifu(${u.id},'shangpingoumai')"/></c:if>
                                    </td>
                                    <td align='center'>${u.issh} </td>
                                    <td height="45" align="center"> ${u.addtime } </td>
                                    <td height="45" align="center"><a class="btn btn-info btn-small"
                                                                      href="../shangpingoumai/doUpdateShangpingoumai.do?id=${u.id }">编辑</a>
                                        <a class="btn btn-info btn-small" href="../shangpingoumai/deleteShangpingoumai.do?id=${u.id }"
                                           onclick="{if(confirm('确定要删除吗?')){return true;}return false;}">删除</a> <a
                                                class="btn btn-info btn-small"
                                                href="../shangpingoumai/shangpingoumaiDetail.do?id=${u.id }">详细</a></td>
                                </tr>
                            </c:forEach>
                        </table>
                    </td>
...
<%
    String sql = "select * from shangpingoumai where gonghao='" + (String) request.getSession().getAttribute("username") + "'  ";
...
<p align="center" class="fy"><c:if test="${sessionScope.p==1 }">
    <c:if test="${page.page>1}">
        <a href="shangpingoumaiList2.do?page=1">首页</a>
        <a href="shangpingoumaiList2.do?page=${page.page-1 }"> 上一页</a> </c:if>
    <c:if test="${page.page<page.totalPage}">
        <a href="shangpingoumaiList2.do?page=${page.page+1 }">下一页</a>
        <a href="shangpingoumaiList2.do?page=${page.totalPage }">末页</a> </c:if>
</c:if>
</p>
